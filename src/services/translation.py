    ...
    for idx, msg in enumerate(anthropic_request.messages):
        ...
        if isinstance(content, str):
            ...
            if msg.role == "user" and any(block.type == "tool_result" for block in content if hasattr(block, "type")):
                ...
                processed_content = []
                for block_idx, block in enumerate(content): # Added block_idx for logging
                    if hasattr(block, "type"):
                        if block.type == "text":
                    ...
                    # else: # This case might occur if content is a list of strings, which is not expected by current models
                        # logger.warning(f"Block {block_idx} in message {idx} does not have a 'type'. Block: {block}")
...
def convert_litellm_to_anthropic(litellm_response: Union[Dict[str, Any], Any], 
    ...
    try:
        ...
        content: List[Union[ContentBlockText, ContentBlockToolUse]] = [] # Type hint for clarity
        ...
        if tool_calls and is_claude_model:
            ...
            for idx, tool_call in enumerate(tool_calls):
                ...
                if isinstance(tool_call, dict):
                    ...
                    arguments_str = function.get("arguments", "{}") # arguments are string from LiteLLM
                else: # Assuming it's an object (e.g. ModelResponse.Choice.Message.ToolCall)
                ...
                if isinstance(arguments_str, str):
                    try:
                        ...
                        arguments = {"raw": arguments_str} # Store raw string if parsing fails
                elif isinstance(arguments_str, dict): # Should not happen based on LiteLLM, but good to handle
            ...
            for idx, tool_call in enumerate(tool_calls):
                ...
                if isinstance(tool_call, dict):
                    function = tool_call.get("function", {})
                    # tool_id = tool_call.get("id", f"tool_{uuid.uuid4()}") # ID not used in text format
                    name = function.get("name", "")
                    arguments = function.get("arguments", "{}")
                else:
                    function = getattr(tool_call, "function", None)
                    # tool_id = getattr(tool_call, "id", f"tool_{uuid.uuid4()}") # ID not used in text format
                    name = getattr(function, "name", "") if function else ""
                    arguments = getattr(function, "arguments", "{}") if function else "{}"
                
                # Convert string arguments to dict if needed, then pretty print
                if isinstance(arguments, str):
                    try:
                        args_dict = json.loads(arguments)
                        arguments_str = json.dumps(args_dict, indent=2)
                    except json.JSONDecodeError:
                        arguments_str = arguments # Use as is if not valid JSON
                elif isinstance(arguments, dict): # Should not happen with LiteLLM
                    arguments_str = json.dumps(arguments, indent=2)
                else:
                    arguments_str = str(arguments) # Fallback
                
                tool_text_parts.append(f"Tool: {name}\nArguments: {arguments_str}\n\n")
            
            tool_text_content = "".join(tool_text_parts)
            # Add or append tool text to content
            if content and content[0].type == "text":
                content[0].text += tool_text_content
            else:
                # If no text content yet, insert this as the first block
                content.insert(0, ContentBlockText(type="text", text=tool_text_content.strip()))

        # Get usage information - extract values safely from object or dict
        if isinstance(usage_info, dict):
            prompt_tokens = usage_info.get("prompt_tokens", 0)
            completion_tokens = usage_info.get("completion_tokens", 0)
        else: # Assuming it's an object (e.g. ModelResponse.Usage)
            prompt_tokens = getattr(usage_info, "prompt_tokens", 0)
            completion_tokens = getattr(usage_info, "completion_tokens", 0)
        
        # Map OpenAI finish_reason to Anthropic stop_reason
        anthropic_stop_reason: Optional[Literal["end_turn", "max_tokens", "stop_sequence", "tool_use"]] = None # Type hint
        ...
        if not content:
            content.append(ContentBlockText(type="text", text="")) # Default empty text block
        
        # Create Anthropic-style response
        anthropic_response = MessagesResponse(
            id=response_id,
            model=original_request.original_model or original_request.model, # Use original model name
            role="assistant", # Role is always assistant for responses
            content=content,
            stop_reason=anthropic_stop_reason,
            stop_sequence=None, # LiteLLM doesn't provide this directly in the same way
            usage=Usage(
        ...
        return MessagesResponse(
            ...
            stop_reason="end_turn", # Or perhaps a new "error" stop_reason if the API supports it

    ...
    try:
        ...
        if anthropic_request.stream:
            if isinstance(orchestrator_response_or_stream, AsyncGenerator):
                ...
                orchestrator_response_or_stream, dict
            ) and 'error' in orchestrator_response_or_stream.get('type', ''):
                # Stream was requested, but orchestrator returned a JSON error (e.g., setup failed before streaming could start)
                logger.error(
                    f"API Endpoint: Orchestrator returned JSON error for a stream request: {orchestrator_response_or_stream}"
                )
                status_code = 500  # Default for pre-stream errors
                return JSONResponse(
                    status_code=status_code, content=orchestrator_response_or_stream
                )
            else:
                logger.error(
            ...
            if isinstance(orchestrator_response_or_stream, AnthropicMessagesResponse):
                ...
                logger.error(
                    f"API Endpoint: Orchestrator returned JSON error for non-stream request: {orchestrator_response_or_stream}"
                )
                ...
                return JSONResponse(
                    status_code=final_status_code, content=orchestrator_response_or_stream
                )
            else:
                logger.error(
